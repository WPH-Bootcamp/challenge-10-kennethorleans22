          ...
          <div className='flex flex-col items-center gap-6 lg:gap-10 w-full max-w-[349px] lg:max-w-[712px]'>
            ...
            <div className='flex items-center gap-[6px] px-4 lg:px-6 bg-white rounded-full w-full h-12 lg:h-14'>
              <Search
                size={20}
                strokeWidth={1.25}
                className='text-[#717680] shrink-0'
              />
              <input
                type='text'
                placeholder='Search restaurants, food and drink'
                className='flex-1 text-sm lg:text-base font-normal text-[#535862] tracking-[-0.02em] outline-none bg-transparent'
              />
            </div>
          </div>
        </div>
  ...
  <div className='grid grid-cols-3 lg:grid-cols-6 gap-5'>
    {categories.map((cat) => (
      <button key={cat.id} type='button' className='flex flex-col items-center gap-1 lg:gap-1.5'>
        ...
        <span className='text-sm xl:text-lg font-bold text-[#0A0D12] tracking-[-0.02em] xl:tracking-[-0.03em] text-center leading-7 xl:leading-8'>
          {cat.label}
        </span>
      </button>
    ))}
...
<section className='flex flex-col items-center px-4 lg:px-[120px] pt-6 pb-12 gap-4 lg:gap-8'>
  <div className='flex items-start justify-between w-full max-w-[1200px]'>
    <h2 className='text-2xl lg:text-[32px] font-extrabold leading-9 lg:leading-[42px] text-[#0A0D12]'>Recommended</h2>
    <Link href='/restaurants' className='text-base lg:text-lg font-extrabold text-[#C12116] tracking-[-0.02em]'>See All</Link>
  </div>
      ...
      restaurants.slice(0, 6).map((restaurant) => (
        ...
        >
          ...
          <div className='flex flex-col gap-0.5 flex-1 min-w-0'>
            <p className='text-base lg:text-lg font-extrabold text-[#0A0D12] tracking-[-0.02em] leading-[30px] lg:leading-8 truncate'>{restaurant.name}</p>
            <div className='flex items-center gap-1'>
              <Star size={24} fill='#FFAB0D' className='text-[#FFAB0D] shrink-0' />
              <span className='text-sm lg:text-base font-medium text-[#0A0D12] leading-7 lg:leading-[30px] lg:tracking-[-0.03em]'>{restaurant.star}</span>
            </div>
            <div className='flex items-center gap-1.5 min-w-0'>
              <span className='text-sm lg:text-base font-normal text-[#0A0D12] tracking-[-0.02em] leading-7 lg:leading-[30px] truncate'>{restaurant.place}</span>
              <span className='w-0.5 h-0.5 rounded-full bg-[#0A0D12] shrink-0' />
              <span className='text-sm lg:text-base font-normal text-[#0A0D12] tracking-[-0.02em] leading-7 lg:leading-[30px] shrink-0'>2.4 km</span>
            </div>
          </div>
        </Link>
      ))
  ...
  <button type='button' className='mt-2 w-[160px] h-10 lg:h-12 border border-[#D5D7DA] rounded-full text-sm lg:text-base font-bold text-[#0A0D12] tracking-[-0.02em]'>Show More</button>
</section>
