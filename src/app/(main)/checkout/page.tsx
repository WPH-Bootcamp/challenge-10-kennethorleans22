...
export default function CheckoutPage() {
  ...
  useEffect(() => {
    ...
    if (defaultAddress?.address) {
      setValue('deliveryAddress', defaultAddress.address);
    }
  }, [addresses, setValue]);
          ...
          >
              ...
              <div className='bg-white rounded-2xl shadow-[0px_0px_20px_rgba(203,202,202,0.25)] p-4 lg:p-5 flex flex-col gap-4 lg:gap-5'>
                ...
                <div className='flex items-center gap-2'>
                  <button
                    ...
                    className='h-9 lg:h-10 px-5 border border-neutral-300 rounded-full flex items-center justify-center cursor-pointer'
                  >
                ...
                <div className='bg-white rounded-2xl shadow-[0px_0px_20px_rgba(203,202,202,0.25)] p-4 lg:p-5 flex flex-col gap-4 lg:gap-5'>
                  <div className='flex flex-row justify-between items-center'>
                    <div className='flex flex-row items-center gap-2 min-w-0'>
                      <img src={cartGroup.restaurant.logo} alt={cartGroup.restaurant.name} className='w-8 h-8 rounded-full object-cover flex-shrink-0' />
                    ...
                    <button type='button' onClick={() => router.push(`/restaurants/${cartGroup.restaurant.id}`)}
                      ...
                      <span className='text-sm lg:text-base font-bold leading-7 lg:leading-[30px] tracking-[-0.02em] text-neutral-950'>Add item</span>
                    </button>
                  </div>
                  ...
                  {cartGroup.items.map((item) => (
                    <div key={item.id} className='flex flex-row justify-between items-center'>
                      <div className='flex flex-row items-center gap-[17px] min-w-0'>
                        <img src={item.menu.image} alt={item.menu.foodName} className='w-16 h-16 lg:w-20 lg:h-20 rounded-xl object-cover flex-shrink-0' />
                        <div className='flex flex-col min-w-0'>
                          <p className='text-sm lg:text-base font-medium leading-7 lg:leading-[30px] lg:tracking-[-0.03em] text-neutral-950 truncate'>{item.menu.foodName}</p>
                          <p className='text-base lg:text-lg font-extrabold leading-[30px] lg:leading-8 lg:tracking-[-0.02em] text-neutral-950'>{formatPrice(item.menu.price)}</p>
                        </div>
                      </div>
                      <div className='flex flex-row items-center gap-4 flex-shrink-0 ml-3'>
                        <button type='button' onClick={() => handleDecrement(item.id, item.quantity)} disabled={isMutating}
                        ...
                        <span className='text-base lg:text-lg font-semibold tracking-[-0.02em] leading-[30px] lg:leading-8 text-neutral-950'>{item.quantity}</span>
                        <button type='button' onClick={() => handleIncrement(item.id, item.quantity)} disabled={isMutating}
            ...
            <div className='lg:w-[390px] lg:flex-shrink-0 bg-white rounded-2xl shadow-[0px_0px_20px_rgba(203,202,202,0.25)] py-4 lg:py-5 flex flex-col gap-4'>
              <div className='flex flex-col gap-3 lg:gap-4 px-4 lg:px-5'>
                <span className='text-base lg:text-lg font-extrabold leading-[30px] lg:leading-8 lg:tracking-[-0.02em] text-neutral-950'>Payment Method</span>
                {PAYMENT_METHODS.map((method, index) => (
                  <div key={method.id}>
                    <button type='button' onClick={() => setValue('paymentMethod', method.id, { shouldValidate: true })}
                      ...
                      <span className='text-sm lg:text-base font-normal leading-7 lg:leading-[30px] tracking-[-0.02em] text-neutral-950 flex-1 text-left'>{method.label}</span>
                      <div className={`w-6 h-6 rounded-full flex-shrink-0 flex items-center justify-center transition-colors ${selectedPayment === method.id ? 'bg-primary-100' : 'border-[1.6px] border-neutral-400'}`}>
                        {selectedPayment === method.id && <div className='w-[10px] h-[10px] bg-white rounded-full' />}
                      </div>
                    </button>
                    {index < PAYMENT_METHODS.length - 1 && <hr className='border-t border-neutral-200 mt-3 lg:mt-4' />}
                  </div>
                ))}
              ...
              <div className='flex flex-col gap-3 lg:gap-4 px-4 lg:px-5'>
                <span className='text-base lg:text-lg font-extrabold leading-[30px] lg:leading-8 lg:tracking-[-0.02em] text-neutral-950'>Payment Summary</span>
                <div className='flex flex-row justify-between items-center'>
                  <span className='text-sm lg:text-base font-medium leading-7 lg:leading-[30px] lg:tracking-[-0.03em] text-neutral-950'>Price ({totalItems} item{totalItems !== 1 ? 's' : ''})</span>
                  <span className='text-sm lg:text-base font-bold leading-7 lg:leading-[30px] tracking-[-0.02em] text-neutral-950'>{formatPrice(subtotal)}</span>
                </div>
                <div className='flex flex-row justify-between items-center'>
                  <span className='text-sm lg:text-base font-medium leading-7 lg:leading-[30px] lg:tracking-[-0.03em] text-neutral-950'>Delivery Fee</span>
                  <span className='text-sm lg:text-base font-bold leading-7 lg:leading-[30px] tracking-[-0.02em] text-neutral-950'>{formatPrice(DELIVERY_FEE)}</span>
                </div>
                <div className='flex flex-row justify-between items-center'>
                  <span className='text-sm lg:text-base font-medium leading-7 lg:leading-[30px] lg:tracking-[-0.03em] text-neutral-950'>Service Fee</span>
                  <span className='text-sm lg:text-base font-bold leading-7 lg:leading-[30px] tracking-[-0.02em] text-neutral-950'>{formatPrice(SERVICE_FEE)}</span>
                </div>
                <div className='flex flex-row justify-between items-center'>
                  <span className='text-base lg:text-lg font-normal leading-[30px] lg:leading-8 text-neutral-950'>Total</span>
                  <span className='text-base lg:text-lg font-extrabold leading-[30px] lg:leading-8 tracking-[-0.02em] text-neutral-950'>{formatPrice(total)}</span>
                </div>

                {checkoutMutation.isError && (
                  <p className='text-xs text-red-500 text-center'>Checkout gagal. Coba lagi.</p>
                )}

                <button type='submit' disabled={checkoutMutation.isPending || !cartGroup}
